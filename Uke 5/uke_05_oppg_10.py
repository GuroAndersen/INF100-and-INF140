...
def mid(x1_or_y1, x2_or_y2, x3_or_y3, a, b, c):
    ...
    """
    midtpunkt = (((x1_or_y1 * a)+(x2_or_y2 * b)+(x3_or_y3 * c))/(a+b+c))
    return midtpunkt
...
def incircle_radius(a, b, c):
    ...
    s = (a+b+c)/2
    radius = round(sqrt((s-a)*((s-b)*(s-c))/s), 16)
    return radius
...
def find_incircle(x1, y1, x2, y2, x3, y3):
    ...
    side1 = side_length(x1, y1, x2, y2)  # side opposite (x3,y3)
    side2 = side_length(x2, y2, x3, y3)  # side opposite (x1,y1)
    side3 = side_length(x3, y3, x1, y1)  # side opposite (x2,y2)
    ...
    radius = incircle_radius (side2, side3, side1)
